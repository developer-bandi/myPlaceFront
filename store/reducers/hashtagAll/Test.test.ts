...
describe("reducer 테스트", () => {
  ...
  it("initial state을 설정한 경우", () => {
    expect(HashtagAllSlice(undefined, {type: "unknown"})).toEqual(initialState);
  });
  ...
  describe("hashtagAllSaga가 의도한대로 작동하는지", () => {
    it("api 를 성공적으로 받아와 액션을 발생시킨 경우", () => {
      return expectSaga(hashtagAllSaga)
        .provide([[call(axiosGetHashTags), {data: apiData}]])
    ...
    it("에러가 발생한 경우", () => {
      return expectSaga(hashtagAllSaga)
        .provide([[call(axiosGetHashTags), throwError(new Error("Whoops"))]])
