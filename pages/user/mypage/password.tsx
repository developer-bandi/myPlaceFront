import Head from "next/head";
import { useRouter } from "next/router";
import Footer from "../../../component/Common/Footer/Footer";
import HeaderContainer from "../../../component/Common/Header/HeaderContainer";
import MyPasswordContainer from "../../../component/User/Mypage/MyPassword/MyPasswordContainer";
import { useLogin } from "../../../lib/customHook/loginCheck";

const Password = () => {
  useLogin();
  const router = useRouter();
  return (
    <>
      <Head>
        <title>My place - 비밀번호 변경</title>
        <meta
          name="title"
          property="og:title"
          content="My place -  비밀번호 변경"
        />
        <meta
          name="description"
          property="og:description"
          content=" 비밀번호 변경할수 있습니다"
        />
        <meta
          name="url"
          property="og:url"
          content={process.env.NEXT_PUBLIC_SERVER_DOMAIN + router.pathname}
        />
      </Head>
      <HeaderContainer />
      <MyPasswordContainer />
      <Footer />
    </>
  );
};

export default Password;
